...
class GPModel(object):
    ...
    def __init__(self, ranges, lengthscale, variance, noise=0.0001, dimension=2, kernel='rbf', period=None):
        ...
        if kernel == 'rbf':
            self.kern = GPy.kern.RBF(input_dim=self.dimension, lengthscale=lengthscale, variance=variance,
                                     ARD=self.asymmetric)
            ...
            self.kern = GPy.kern.StdPeriodic(input_dim=self.dimension, period=period, lengthscale=lengthscale,
                                             variance=variance, ARD1=True, ARD2=True) \
                        + GPy.kern.RBF(input_dim=self.dimension, lengthscale=lengthscale, variance=variance,
                                       ARD=self.asymmetric)
        else:
    ...
    def add_data(self, xvals, zvals):
        ...
        if update_legacy:
            ...
            if self.model == None:
                ...
                self.model = GPy.models.GPRegression(np.array(self.temp_xvals), np.array(self.temp_zvals), self.kern,
                                                     noise_var=self.noise)
            # Else add to the exisiting model
            else:
...
class SubsampledGPModel(OnlineGPModel):
    ...
    def __init__(self, ranges, lengthscale, variance, noise=0.0001, dimension=2, kernel='rbf', update_legacy=False,
                 max_size=20, neighbor_radius=1.00, val_eps=2.00):
    ...
    def add_data(self, xvals, zvals):
        ...
        if self.xvals is None:
            ...
            print
            "Distance to nearest neighbor:", dist
            print
            "Index:", index
            print
            "Dataset:", self.xvals.shape

            for j, (d, i) in enumerate(zip(dist, index)):
                m, v = self.predict_value(xvals, include_noise=True, full_cov=False)
                print
                "Value distance:", np.abs(self.zvals[i, :] - m[j, :])
                if d == float("inf") or np.abs(self.zvals[i, :] - m[j, :]) > self.val_eps:
                    print
                    "Udating model with point:", d, self.xvals[i], "and values:", self.zvals[i, :], m[j, :]
                    self.update_model(xvals, zvals)
                    return
            print
            "---------- Skiped Update! -------------------"
