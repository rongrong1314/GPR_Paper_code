...
class MCTS(object):
    ...
    def __init__(self, computation_budget, belief, initial_pose, rollout_length, path_generator, aquisition_function,
                 f_rew, T, aq_param=None, use_cost=False, tree_type=None, num_samples=20):
        '''
    ...
    def choose_trajectory(self, t):
        ...
        time_end = time.time()
        print
        "Rollouts completed in", str(time_end - time_start) + "s",
        ...
        print
        "Number of rollouts:", i, "\t Size of tree:", len(self.tree)
        logger.info("Number of rollouts: {} \t Size of tree: {}".format(i, len(self.tree)))
        np.save('./figures/' + self.f_rew + '/tree_' + str(t) + '.npy', self.tree)
        return self.tree[best_sequence][0], self.tree[best_sequence][
            1], best_val, paths, all_vals, self.max_locs, self.max_val
    ...
    def tree_policy(self):
        ...
        for i, val in actions.items():
            ...
            if self.tree[node][4] == 0:
                ...
                leaf_eval[node] = self.tree[node][3] + self.c * np.sqrt(
                    2 * (np.log(self.tree['root'][1])) / self.tree[node][4])
    ...
    def rollout_policy(self, node):
        ...
        for i in xrange(self.rl):
            actions, dense_paths = self.path_generator.get_path_set(
                self.tree[node][0][-1])  # plan from the last point in the sample
            # check that paths were generated; if not, roll back if possible
            try:
                keys = actions.keys()
            except:
                print
                'No actions were viably generated; rolling back'
                sequence.remove(node)
                if len(sequence) == 0:
                    print
                    "Empty sequence ", sequence, node
            ...
            try:
                self.tree[node + ' child ' + str(keys[a])] = (
                actions[keys[a]], dense_paths[keys[a]], 0, 0, 0)  # add random path to the tree
    ...
    def get_reward(self, sequence):
        ...
        if self.f_rew == 'maxs-mes':
            reward = self.aquisition_function(time=self.t, xvals=obs, robot_model=self.GP,
...
class Tree(object):
        ...
        # At random node, after selected action from a specific node
            ...
            if self.f_rew == 'mes' or self.f_rew == 'maxs-mes':
                ...
                r = self.aquisition_function(time=self.t, xvals=xobs, robot_model=belief,
                                             param=self.param)  # (param, self.param[1]))
            elif self.f_rew == 'naive_value':
                r = self.aquisition_function(time=self.t, xvals=xobs, robot_model=belief, param=self.param)
            else:
                r = self.aquisition_function(time=self.t, xvals=xobs, robot_model=belief)
            ...
            if current_node.children is not None:
                ...
                if current_node.depth < self.max_depth - 1 and np.floor(nchild ** alpha) == np.floor(
                        (nchild - 1) ** alpha):
    ...
    def get_next_child(self, current_node):
        ...
        for i, child in enumerate(current_node.children):
            ...
            if child.nqueries == 0:
                return child
            vals[child] = child.reward / float(child.nqueries) + self.c * np.sqrt(
                (float(current_node.nqueries) ** e_d) / float(child.nqueries))
    ...
    def get_next_child(self, current_node):
        ...
        for i, child in enumerate(current_node.children):
            ...
            if child.nqueries == 0:
                return child, False
            vals[child] = child.reward / float(child.nqueries) + self.c * np.sqrt(
                2.0 * np.log(float(current_node.nqueries)) / float(child.nqueries))
            # Return the max node, or a random node if the value is equal
