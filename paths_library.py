...
class Dubins_Path_Generator(Path_Generator):
    ...
    def buffered_paths(self):
        ...
        for i, goal in enumerate(self.goals):
            ...
            ftemp = []
            for c in fconfig:
                if c[0] > self.extent[0] and c[0] < self.extent[1] and c[1] > self.extent[2] and c[1] < self.extent[
                    3] and not self.obstacle_world.in_obstacle((c[0], c[1]), buff=0.0):
                    ftemp.append(c)
                else:
                    break
    ...
    def make_sample_paths(self):
        ...
        for i, goal in enumerate(self.goals):
            ...
            coords[i] = [config for config in configurations if
                         ...
                         config[1] < self.extent[3] and not self.obstacle_world.in_obstacle((config[0], config[1]),
                                                                                            buff=self.tr)]
    ...
    def take_step(self, loc):
        ...
        for i, goal in enumerate(self.goals):
            ...
            if dist < self.step_size:
                ...
                ftemp = []
                for c in fconfig:
                    if c[0] > self.ranges[0] and c[0] < self.ranges[1] and c[1] > self.ranges[2] and c[1] < self.ranges[
                        3] and not self.obstacle_world.in_obstacle((c[0], c[1]), buff=self.turning_radius):
                        ftemp.append(c)
                    else:
                        break
                ...
                try:
                    ttemp = ftemp[0::10]
                    for m, c in enumerate(ttemp):
                        if c[0] < self.ranges[0] + 3 * self.turning_radius or c[0] > self.ranges[
                            1] - 3 * self.turning_radius or c[1] < self.ranges[2] + 3 * self.turning_radius or c[1] > \
                                self.ranges[3] - 3 * self.turning_radius or self.obstacle_world.in_obstacle(
                                (c[0], c[1]), buff=3 * self.turning_radius):
                            ttemp = ttemp[0:m - 1]
...
class Reachable_Step_Generator(Reachable_Frontier_Generator):
    ...
    def take_step(self, loc):
        ...
        for i, goal in enumerate(self.goals):
            ...
            if dist > self.step_size:
                new_goal = (loc[0] + self.step_size * np.sin(np.pi / 2 - angle_to_goal),
                            loc[1] + self.step_size * np.sin(angle_to_goal), angle_to_goal)
            else:
                new_goal = (goal[0], goal[1], angle_to_goal)
            ...
            for config in configurations:
                if config[0] > self.ranges[0] and config[0] < self.ranges[1] and config[1] > self.ranges[2] and config[
                    1] < self.ranges[3] and not self.obstacle_world.in_obstacle((config[0], config[1]),
